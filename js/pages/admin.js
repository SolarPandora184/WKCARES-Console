...
import { applyScriptTemplate, makeScriptKey, isScriptKey, scriptIdFromKey } from "../utils/script.js";
import {
  adminLevel,
...
onAuthReady((user, profile) => {
  ...
  if (!hasRole("admin_1", "admin_2", "admin_3")) {
          ...
          <tr class="${u.disabled ? "is-muted-row" : ""}" data-uid="${u.id}">
            ...
            <td>
              <select class="user-role-select" data-uid="${u.id}"
                ${!canEditUserRoles() || u.id === currentProfile?.uid ? "disabled" : ""}
                ${u.id === currentProfile?.uid ? 'title="You can\'t change your own role"' : !canEditUserRoles() ? 'title="Level 2 or higher can change roles"' : ""}>
            ...
            <td>
              ...
              <div class="row-actions">
                ...
                <button type="button" class="btn btn--secondary user-toggle-btn" data-uid="${u.id}" data-disabled="${!!u.disabled}"
                  ...
                  : `<span style="color:var(--color-text-muted); font-size:0.78rem;">Level 2+ required</span>`
              }
            </td>
          </tr>`
          )
          ...
          <tr>
            ...
            <td>
              <div class="row-actions">
                ...
                ${
                  actionable && canManageInvitations()
                    ? `
                ...
                ${
                  (status.label === "Revoked" || status.label === "Expired") && canManageInvitations()
                    ? `
...
function initStaffTab() {
  ...
  document.getElementById("staff-save-btn").hidden = !editable;
  if (!editable) setSaved("staff", "Level 2 or higher can edit the roster");
  ...
  watchStaff((rows) => {
    ...
    if (editable) setSaved("staff", `${rows.length} staff members`);
  });
...
function initAreasTab() {
  const editable = canEditRoster();
  document.getElementById("areas-save-btn").hidden = !editable;

  // Mountain Areas and Metro/City Areas are stored under separate paths
  // (areas/mountain, areas/metro) so they can never collide with each other
  // — each feeds its own grid on the Weekly Net Form.
  const columns = [
  ...
  const emptyRow = { name: "", callsign: "", frequency: "", active: true };

  watchAreas("mountain", (rows) => {
    mountainOriginalIds = new Set(rows.map((r) => r.id));
    const initialRows = rows.map((r) => ({ id: r.id, name: r.name ?? "", callsign: r.callsign ?? "", frequency: r.frequency ?? "", active: r.active !== false }));
  ...
  watchAreas("metro", (rows) => {
    ...
    metroTable = new DynamicTable(document.getElementById("metro-areas-table"), columns, initialRows, () => setUnsaved("areas"), {
      addLabel: "Add metro/city area",
      emptyRow,
      readOnly: !editable,
    });
    setSaved("areas", editable ? "Areas loaded" : "Level 2 or higher can edit areas");
  });
...
function initBandsTab() {
  const editable = canEditRoster();
  document.getElementById("bands-save-btn").hidden = !editable;
  ...
  watchBands((rows) => {
    ...
    setSaved("bands", editable ? `${rows.length} bands configured` : "Level 2 or higher can edit bands");
  });
...
/* ---------------------------------------------------------------------- */
/* Net Script — every script is hand-made. There are no built-in/premade   */
/* scripts; Level 3 admins create each one (Intro, ID reminders, emergency */
/* traffic prompts, closing, etc.) as its own section here, then place it  */
/* on the form (possibly more than once, as separate sections) in the     */
/* Form Layout tab.                                                        */
/* ---------------------------------------------------------------------- */
...
function initScriptTab() {
  const editable = canEditScript();
  const addSectionBtn = document.getElementById("script-add-section-btn");
  addSectionBtn.hidden = !editable;
  document.getElementById("script-readonly-hint").hidden = editable;

  watchNetScript((script) => {
    latestScript = script;
    renderScriptSections(script.sections);
  });

  if (editable) {
    addSectionBtn.addEventListener("click", async () => {
      const label = window.prompt("Section name (shown as the label on the Weekly Net Form, e.g. \"Net Opening\" or \"Emergency Traffic Prompt\"):");
      if (!label) return;
      try {
        await saveScriptSection(null, { label: label.trim(), text: "" });
        showToast("Script section added. Edit its wording below, then place it in the Form Layout tab.", "success");
      } catch (err) {
        showToast(err.message || "Failed to add script section.", "error");
      }
    });
  }
}

function renderScriptSections(sections) {
  const container = document.getElementById("script-sections");
  const editable = canEditScript();
  const entries = Object.entries(sections || {});

  if (!entries.length) {
    container.innerHTML = `<div class="empty-state">No script sections yet. ${editable ? 'Click "Add script section" below to create the first one.' : ""}</div>`;
    return;
  }
  ...
  container.innerHTML = entries
    .map(
      ([id, section]) => `
    <div class="field" data-section-id="${id}" style="border:1px solid var(--color-border); border-radius:var(--radius-md); padding:12px; margin-bottom:10px;">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
        <label style="margin:0;">${escapeHtml(section.label)}</label>
        ${
          editable
            ? `<button type="button" class="btn btn--icon section-delete" data-id="${id}" title="Delete section">
          <span class="material-icons">delete_outline</span></button>`
            : ""
        }
      </div>
      <textarea rows="2" class="section-text" data-id="${id}" ${editable ? "" : "readonly"}>${escapeHtml(section.text)}</textarea>
    </div>`
    )
    .join("");
  ...
  container.querySelectorAll(".section-text").forEach((ta) => {
    ta.addEventListener("blur", async () => {
      const id = ta.dataset.id;
      const label = latestScript?.sections?.[id]?.label ?? "";
      try {
        await saveScriptSection(id, { label, text: ta.value.trim() });
  ...
  container.querySelectorAll(".section-delete").forEach((btn) => {
    btn.addEventListener("click", async () => {
      if (!window.confirm("Delete this script section? Remove it from the Form Layout tab first if it's currently placed on the form.")) return;
      try {
        await deleteScriptSection(btn.dataset.id);
...
/* ---------------------------------------------------------------------- */
/* Net Form layout (Level 3 only)                                          */
/* ---------------------------------------------------------------------- */

let currentLayoutOrder = [...DEFAULT_NET_FORM_ORDER];

const BUILTIN_CARD_LABELS = {
  header: "Net Header (Controller / Callsign / Date / Comments)",
  staff: "Staff Comments / Check-ins",
  mountain: "Mountain Areas (Green/Yellow/Red/Black check-ins)",
  "bands-summary": "Alternate Bands — per-band summary & count",
  "bands-individual": "Alternate Bands — individual check-ins",
  emergency: "Emergency Traffic (entry table)",
  metro: "Metro / City Areas (Green/Yellow/Red/Black check-ins)",
  guests: "Guest Check-ins",
};

function cardLabel(key) {
  if (BUILTIN_CARD_LABELS[key]) return BUILTIN_CARD_LABELS[key];
  if (isScriptKey(key)) {
    const id = scriptIdFromKey(key);
    return `Script: ${latestScript?.sections?.[id]?.label ?? "(deleted section)"}`;
  }
  return key;
}

function initFormLayoutTab() {
  const editable = canEditFormLayout();
  document.getElementById("form-layout-hint").hidden = editable;
  document.getElementById("form-layout-add-row").hidden = !editable;

  watchNetFormLayout((layout) => {
    currentLayoutOrder = layout.order;
    renderFormLayoutList();
  });

  // Also re-render when script data changes, since section labels shown
  // in the list come from netScript.sections.
  watchNetScript(() => renderFormLayoutList());

  initFormLayoutAddButton();
}

function renderFormLayoutList() {
  const container = document.getElementById("form-layout-list");
  const editable = canEditFormLayout();

  container.innerHTML = currentLayoutOrder
    .map(
      (key, i) => `
    <div class="form-layout-row" data-key="${escapeAttr(key)}">
      <span class="material-icons" aria-hidden="true" style="color:var(--color-text-muted);">drag_indicator</span>
      <span style="flex:1;">${escapeHtml(cardLabel(key))}</span>
      ${
        editable
          ? `
      <button type="button" class="btn btn--icon layout-move-up" data-index="${i}" ${i === 0 ? "disabled" : ""} title="Move up">
        <span class="material-icons">arrow_upward</span>
      </button>
      <button type="button" class="btn btn--icon layout-move-down" data-index="${i}" ${i === currentLayoutOrder.length - 1 ? "disabled" : ""} title="Move down">
        <span class="material-icons">arrow_downward</span>
      </button>
      ${
        isScriptKey(key)
          ? `<button type="button" class="btn btn--icon layout-remove" data-index="${i}" title="Remove from form">
  ...
  container.querySelectorAll(".layout-move-up").forEach((btn) =>
    btn.addEventListener("click", () => moveLayoutItem(Number(btn.dataset.index), -1))
  );
  container.querySelectorAll(".layout-move-down").forEach((btn) =>
    btn.addEventListener("click", () => moveLayoutItem(Number(btn.dataset.index), 1))
  );
  container.querySelectorAll(".layout-remove").forEach((btn) =>
    btn.addEventListener("click", () => {
      currentLayoutOrder.splice(Number(btn.dataset.index), 1);
      renderFormLayoutList();
      persistLayout();
    })
  );

  const addSelect = document.getElementById("form-layout-add-select");
  const placedIds = new Set(currentLayoutOrder.filter(isScriptKey).map(scriptIdFromKey));
  const availableSections = Object.keys(latestScript?.sections || {}).filter((id) => !placedIds.has(id));
  addSelect.innerHTML = availableSections.length
    ? availableSections.map((id) => `<option value="${id}">${escapeHtml(latestScript.sections[id].label)}</option>`).join("")
    : `<option value="">No unplaced script sections — create another in the Script tab if you need this wording twice</option>`;
}

function moveLayoutItem(index, delta) {
  const target = index + delta;
  if (target < 0 || target >= currentLayoutOrder.length) return;
  [currentLayoutOrder[index], currentLayoutOrder[target]] = [currentLayoutOrder[target], currentLayoutOrder[index]];
  renderFormLayoutList();
  persistLayout();
}
...
async function persistLayout() {
  try {
    await saveNetFormLayout(currentLayoutOrder);
  } catch (err) {
    showToast(err.message || "Failed to save form layout.", "error");
  }
}

function initFormLayoutAddButton() {
  const btn = document.getElementById("form-layout-add-btn");
  if (!btn) return;
  btn.addEventListener("click", () => {
    const select = document.getElementById("form-layout-add-select");
    const id = select.value;
    if (!id) return showToast("No script sections available to add. Create one in the Script tab first.", "error");
    currentLayoutOrder.push(makeScriptKey(id));
    renderFormLayoutList();
    persistLayout();
  });
}
