...
/* prompt and the "ID yourself" reminder). Level 3 admins only can add     */
/* their own custom script sections beyond these two built-in ones — see  */
/* js/utils/permissions.js#canEditScript.                                 */
/* ---------------------------------------------------------------------- */
...
export function watchNetScript(callback) {
  return onValue(ref(db, "netScript"), (snap) => {
    const val = snap.val() || {};
    callback({ ...DEFAULT_NET_SCRIPT, ...val, customSections: val.customSections || {} });
  });
}

/** Saves just the two built-in fields — leaves customSections untouched. */
export async function saveNetScriptText(data) {
  await update(ref(db, "netScript"), data);
  await logAudit("net_script_edit", { data });
}

/** Creates (id === null) or updates (id given) one custom script section. */
export async function saveCustomScriptSection(id, data) {
  const path = id ? `netScript/customSections/${id}` : `netScript/customSections/${push(ref(db, "netScript/customSections")).key}`;
  await set(ref(db, path), data);
  await logAudit("net_script_section_edit", { path, data });
  return path.split("/").pop();
}

export async function deleteCustomScriptSection(id) {
  await remove(ref(db, `netScript/customSections/${id}`));
  await logAudit("net_script_section_delete", { id });
}

/* ---------------------------------------------------------------------- */
/* Net Form layout (Level 3 admins only) — the order in which the Weekly  */
/* Net Form's cards appear, including where any custom script sections    */
/* (keys prefixed "custom:") are inserted among the built-in cards.       */
/* ---------------------------------------------------------------------- */
