...
import { applyScriptTemplate, isScriptKey, scriptIdFromKey } from "../utils/script.js";
import {
  totalStaffPresent,
  totalBandCheckins,
  gridTotals,
...
const state = {
  ...
  mountain: [], // [{ area, frequency, green, yellow, red, black }] — one row per controller area
  metro: [],
  bandsSummary: [], // DynamicTable rows: band, mode, controllerName, controllerCallsign, frequency, count
  bandsIndividual: [], // DynamicTable rows: band, name, callsign, location, notes
...
let bandsSummaryTable, bandsIndividualTable, guestsTable, emergencyTable;
let referenceBands = []; // from Admin → Bands, feeds the Band/Mode dropdowns below
...
async function bootstrap(profile) {
  ...
  // 3. Staff comments/check-ins: merge live roster with any saved attendance
  //    for this net. What staff say isn't logged — only whether they checked in.
  ...
  //    each carrying its Frequency along for reference (not part of the totals).
  ...
  // 5. Alternate Bands reference list (Admin → Bands) drives the Band/Mode
  //    dropdowns in both band tables below. Rebuild those tables whenever it
  //    changes so the dropdown options stay current.
  watchBands((rows) => {
    referenceBands = rows.filter((r) => r.active !== false);
    renderBandsSummaryTable();
    renderBandsIndividualTable();
  });
  ...
  // 8. Net Script (every section is hand-made — see Admin → Script) and the
  //    Level-3-controlled card order (Admin → Form Layout). Both are
  //    re-applied whenever either changes, or whenever the net's own
  //    Callsign field changes — since script text can contain "(ID)", which
  //    is swapped for the current net's callsign every time any of that updates.
  watchNetScript((script) => {
    latestScript = script;
    renderScriptTexts(); // creates/fills every script banner, then reorders
  });
  watchNetFormLayout((layout) => {
    latestLayoutOrder = layout.order;
    renderScriptTexts(); // re-run so any newly-ordered banners exist before reordering
  });
...
/* ---------------------------------------------------------------------- */
/* Alternate Bands — two linked sub-sections:                             */
/*  (a) Band Summary: one row per band/mode in use, with a headcount       */
/*  (b) Individual Check-ins: who checked in, on which band                */
/* The section total is the sum of the Summary rows' Count field — plain   */
/* numbers, not color-coded like the Mountain/Metro grids.                 */
/* ---------------------------------------------------------------------- */

function bandNameOptions() {
  const names = referenceBands.map((b) => b.band).filter(Boolean);
  return names.length ? names : ["(no bands configured in Admin)"];
}

function renderBandsSummaryTable() {
  const columns = [
    { key: "band", label: "Band", type: "select", options: bandNameOptions() },
    { key: "mode", label: "Mode", type: "select", options: ["Voice", "Digital", "CW"] },
    { key: "controllerName", label: "Controller Name", type: "text" },
    { key: "controllerCallsign", label: "Controller Callsign", type: "text" },
    { key: "frequency", label: "Frequency", type: "text" },
    { key: "count", label: "Count", type: "number" },
  ];
  bandsSummaryTable = new DynamicTable(
    document.getElementById("bands-summary-table"),
    columns,
    state.bandsSummary,
    (rows) => {
      state.bandsSummary = rows;
      recomputeTotals();
    },
    {
      addLabel: "Add band",
      emptyRow: { band: bandNameOptions()[0], mode: "Voice", controllerName: "", controllerCallsign: "", frequency: "", count: 0 },
    }
  );
}

function renderBandsIndividualTable() {
  const columns = [
    { key: "band", label: "Band", type: "select", options: bandNameOptions() },
    { key: "name", label: "Name", type: "text" },
    { key: "callsign", label: "Callsign", type: "text" },
  ...
  bandsIndividualTable = new DynamicTable(
    document.getElementById("bands-individual-table"),
    columns,
    state.bandsIndividual,
    (rows) => {
      state.bandsIndividual = rows;
      // Individual check-ins are a detail log, not part of the numeric total
      // (the Band Summary's Count column is), but still worth autosaving.
      scheduleAutosave();
    },
    {
      addLabel: "Add individual check-in",
      emptyRow: { band: bandNameOptions()[0], name: "", callsign: "", location: "", notes: "" },
    }
  );
}
...
/** Ensures every script section referenced in the layout order has a DOM
 *  banner element (creating any that don't exist yet), fills in its text
 *  with the "(ID)" substitution applied, then reorders the cards to match
 *  the stored layout. Re-run whenever the script, the layout, or the net's
 *  callsign changes. */
function renderScriptTexts() {
  ...
  const sections = latestScript?.sections || {};

  for (const key of latestLayoutOrder) {
    if (!isScriptKey(key)) continue;
    const id = scriptIdFromKey(key);
    const section = sections[id];
    if (!section) continue; // section was deleted but the layout wasn't updated yet
...
/** Renders a Mountain/Metro grid: rows = area controllers (with their
 *  Frequency shown for reference), columns = Green/Yellow/Red/Black, with
 *  live row totals, column totals, and a grand total footer. */
function renderAreaGrid(containerId, rows, stateKey) {
  ...
  container.innerHTML = `
    <table class="data-table">
      <thead>
        <tr><th>Area</th><th>Frequency</th>${cols.map((c) => `<th>${capitalize(c)}</th>`).join("")}<th>Row total</th></tr>
      </thead>
      <tbody>
        ${rows
          .map(
          ...
          <tr data-row="${i}">
            ...
            <td class="area-frequency">${escapeHtml(r.frequency || "—")}</td>
            ${cols
              .map(
...
function recomputeTotals() {
  ...
  const bandsTotal = totalBandCheckins(state.bandsSummary);
