...
import {
  ...
  watchNetScript,
  saveNetScriptText,
  saveCustomScriptSection,
  deleteCustomScriptSection,
  watchNetFormLayout,
  saveNetFormLayout,
...
function initScriptTab() {
  ...
  const emergencyField = document.getElementById("script-emergency");
  const idYourselfField = document.getElementById("script-id-yourself");
  const saveBtn = document.getElementById("script-save-btn");
  const addSectionBtn = document.getElementById("script-add-section-btn");

  emergencyField.readOnly = !editable;
  idYourselfField.readOnly = !editable;
  saveBtn.hidden = !editable;
  addSectionBtn.hidden = !editable;
  if (!editable) setSaved("script", "Level 3 only can edit the script");

  watchNetScript((script) => {
    latestScript = script;
    // Only overwrite the fields if the admin hasn't started typing an edit —
    // avoids clobbering an in-progress edit if this fires again mid-session.
    if (document.activeElement !== emergencyField) emergencyField.value = script.emergencyPrompt;
    if (document.activeElement !== idYourselfField) idYourselfField.value = script.idYourself;
    renderCustomSections(script.customSections);
  });
  ...
  if (editable) {
    saveBtn.addEventListener("click", async () => {
      setSaving("script");
      try {
        await saveNetScriptText({
          emergencyPrompt: emergencyField.value.trim(),
          idYourself: idYourselfField.value.trim(),
        });
        setSaved("script", "Saved");
        showToast("Net script saved.", "success");
      } catch (err) {
        setSaved("script", "Save failed");
        showToast(err.message || "Failed to save net script.", "error");
      }
    });

    addSectionBtn.addEventListener("click", async () => {
      const label = window.prompt("Section name (shown as the label on the Weekly Net Form, e.g. \"Net Opening\"):");
      if (!label) return;
      try {
        await saveCustomScriptSection(null, { label: label.trim(), text: "" });
        showToast("Script section added. Edit its wording below, then add it to the Form Layout tab to place it.", "success");
      } catch (err) {
...
function renderCustomSections(customSections) {
  const container = document.getElementById("script-custom-sections");
  const editable = canEditScript();
  const entries = Object.entries(customSections || {});

  if (!entries.length) {
    container.innerHTML = `<div class="empty-state">No custom script sections yet.</div>`;
    return;
  }
  ...
  container.innerHTML = entries
    ...
    <div class="field" data-section-id="${id}" style="border:1px solid var(--color-border); border-radius:var(--radius-md); padding:12px; margin-bottom:10px;">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
        ...
        ${
          editable
            ? `<button type="button" class="btn btn--icon custom-section-delete" data-id="${id}" title="Delete section">
          <span class="material-icons">delete_outline</span></button>`
            : ""
        }
      </div>
      <textarea rows="2" class="custom-section-text" data-id="${id}" ${editable ? "" : "readonly"}>${escapeHtml(section.text)}</textarea>
    </div>`
    )
    .join("");
  ...
  container.querySelectorAll(".custom-section-text").forEach((ta) => {
    ta.addEventListener("blur", async () => {
      const id = ta.dataset.id;
      const label = latestScript?.customSections?.[id]?.label ?? "";
      try {
        await saveCustomScriptSection(id, { label, text: ta.value.trim() });
  ...
  container.querySelectorAll(".custom-section-delete").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      try {
        await deleteCustomScriptSection(btn.dataset.id);
...
const BUILTIN_CARD_LABELS = {
  ...
  staff: "Staff Check-ins",
  mountain: "Mountain Areas",
  bands: "Alternate Bands",
  emergency: "Emergency Traffic (with entry table)",
  "script-idYourself": "Script: \"ID yourself\" reminder",
  metro: "Area Controllers (Metro)",
  "script-emergencyPrompt-2": "Script: Emergency prompt (2nd reading)",
...
function cardLabel(key) {
  ...
  if (key.startsWith("custom:")) {
    ...
    return `Script: ${latestScript?.customSections?.[id]?.label ?? "(deleted section)"}`;
  }
  return key;
}
...
function initFormLayoutTab() {
  ...
  // Also re-render when script data changes, since custom-section labels
  // shown in the list come from netScript.customSections.
...
function renderFormLayoutList() {
  ...
  const availableCustom = Object.keys(latestScript?.customSections || {}).filter(
    (id) => !currentLayoutOrder.includes(`custom:${id}`)
  );
  addSelect.innerHTML = availableCustom.length
    ? availableCustom.map((id) => `<option value="${id}">${escapeHtml(latestScript.customSections[id].label)}</option>`).join("")
    : `<option value="">No unplaced script sections</option>`;
}
...
function initFormLayoutAddButton() {
  ...
  btn.addEventListener("click", () => {
    ...
    currentLayoutOrder.push(`custom:${id}`);
    renderFormLayoutList();
    persistLayout();
  });
}
