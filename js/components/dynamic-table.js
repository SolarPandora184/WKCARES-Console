...
export class DynamicTable {
  ...
  constructor(container, columns, initialRows, onChange, opts = {}) {
    ...
    // When true, renders every cell disabled and hides the add/remove controls —
    // used for admin tiers that can view a roster but not edit it.
  ...
  render() {
    const showControls = !this.readOnly;
    const theadCells = this.columns.map((c) => `<th>${c.label}</th>`).join("") + (showControls ? "<th></th>" : "");
    ...
    if (!showControls) return; // read-only: no listeners to wire up
  ...
  renderCell(row, col, rowIndex) {
    ...
    if (col.type === "checkbox") {
      return `<input type="checkbox" data-field="${col.key}" ${val ? "checked" : ""} ${disabled} />`;
    }
    if (col.type === "select") {
      const opts = (col.options || [])
        .map((o) => `<option value="${o}" ${o === val ? "selected" : ""}>${o}</option>`)
        .join("");
      return `<select data-field="${col.key}" ${disabled}>${opts}</select>`;
    }
    return `<input type="${col.type}" data-field="${col.key}" value="${escapeAttr(val)}" ${col.type === "number" ? 'min="0" step="1"' : ""} ${disabled} />`;
  }
}
