...
import { watchStaff, watchAreas, autosaveNetDraft, updateNet, getNet, watchNetScript } from "../firebase/db.js";
...
async function bootstrap(profile) {
  ...
  // Net script text (admin-editable, from Admin Panel → Script). Rendered at
  // both checkpoints from the same source so an edit updates both at once.
  watchNetScript((script) => {
    document.getElementById("script-emergency-1").textContent = script.emergencyPrompt;
    document.getElementById("script-emergency-2").textContent = script.emergencyPrompt;
    document.getElementById("script-id-yourself").textContent = script.idYourself;
  });
