...
/* Form). Every script is hand-made — there are no built-in/premade ones. */
/* Level 3 admins create, edit, and delete sections; Form Layout controls */
/* where (and whether) each one appears on the form.                      */
/* See js/utils/permissions.js#canEditScript / canEditFormLayout.         */
/* ---------------------------------------------------------------------- */

export function watchNetScript(callback) {
  return onValue(ref(db, "netScript"), (snap) => {
    const raw = snap.val() || {};
    const sections = { ...(raw.sections || raw.customSections || {}) };

    // One-time-friendly migration: if this net previously used the old
    // built-in emergencyPrompt/idYourself fields and actually saved text to
    // them, carry that wording over as regular sections so it isn't lost.
    // Nothing is created if those fields were never actually saved.
    if (raw.emergencyPrompt && !sections.__legacyEmergencyPrompt) {
      sections.__legacyEmergencyPrompt = { label: "Emergency Traffic Prompt (migrated)", text: raw.emergencyPrompt };
    }
    if (raw.idYourself && !sections.__legacyIdYourself) {
      sections.__legacyIdYourself = { label: "ID Yourself Reminder (migrated)", text: raw.idYourself };
    }
...
/** Creates (id === null) or updates (id given) one script section. */
export async function saveScriptSection(id, data) {
  const path = id ? `netScript/sections/${id}` : `netScript/sections/${push(ref(db, "netScript/sections")).key}`;
  await set(ref(db, path), data);
  await logAudit("net_script_section_edit", { path, data });
  return path.split("/").pop();
}

export async function deleteScriptSection(id) {
  await remove(ref(db, `netScript/sections/${id}`));
  await logAudit("net_script_section_delete", { id });
}

/* ---------------------------------------------------------------------- */
/* Net Form layout (Level 3 admins only) — the order in which the Weekly  */
/* Net Form's cards appear, including where any script sections (keys     */
/* prefixed "script:") are inserted among the structural data-entry cards.*/
/* Scripts are never built into this list by default — the admin places  */
/* every one by hand in the Form Layout tab, matching how they're hand-   */
/* made in the Script tab.                                                */
/* ---------------------------------------------------------------------- */

export const DEFAULT_NET_FORM_ORDER = [
  "header",
  "staff",
  "mountain",
  "bands-summary",
...
export function watchNetFormLayout(callback) {
  return onValue(ref(db, "netFormLayout"), (snap) => {
    const val = snap.val();
    callback({ order: val?.order?.length ? val.order : [...DEFAULT_NET_FORM_ORDER] });
  });
}

export async function saveNetFormLayout(order) {
  await set(ref(db, "netFormLayout"), { order });
  await logAudit("net_form_layout_edit", { order });
}
...
export async function exportFullBackup() {
  const nodes = ["staff", "areas", "bands", "nets", "users", "invitations", "auditLog", "netScript", "netFormLayout"];
